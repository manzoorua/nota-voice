-- Schema Consolidation: Migrate from legacy to clean tables (Fixed enum casting)
...
-- 4. Migrate subscriptions data with proper enum casting
...
SELECT 
  ...
  CASE 
    ...
    WHEN status::text = 'unpaid' THEN 'unpaid'::clean_subscription_status
    WHEN status::text = 'incomplete' THEN 'incomplete'::clean_subscription_status
...
-- 7. Migrate voice_notes data (legacy -> clean_voice_notes) - only if voice_notes table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'voice_notes' AND table_schema = 'public') THEN
    ...
    SELECT 
      ...
      duration_seconds,
      CASE 
        WHEN status::text = 'completed' THEN 'completed'::clean_voice_note_status
        WHEN status::text = 'processing' THEN 'processing'::clean_voice_note_status
        WHEN status::text = 'failed' THEN 'error'::clean_voice_note_status
...
-- 11. Create compatibility views for gradual migration
...
SELECT 
  id,
  user_id,
  status::text::subscription_status as status,
  plan_name,
  current_period_start,
  current_period_end,
